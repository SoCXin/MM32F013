...
void SPIM_Init(u16 spi_baud_div)
{
    ...
    //PB12 as General push-pull output
    GPIOB->CRH |=  ((GPIO_CNF_MODE_OUT_PP << GPIO_CRH_CNF_MODE_12_Pos));

    //PB15,PB13 as AF push-pull output
    ...
    //set PB13,PB14,PB15 to default AF value
    GPIOB->AFRH &= ~((GPIO_AF_MODEMASK << GPIO_AFRH_AFR13_Pos) | \
                     (GPIO_AF_MODEMASK << GPIO_AFRH_AFR14_Pos) | \
                     (GPIO_AF_MODEMASK << GPIO_AFRH_AFR15_Pos)) ;

    //set PB13,PB14,PB15 to AF0
    GPIOB->AFRH |=  ((GPIO_AF_MODE0 << GPIO_AFRH_AFR13_Pos) | \
