...
u32 SPIMReadWriteByte(u8 tx_data)
{
    SPI2->TXREG = tx_data;
    while (1) {
        if((SPI2->CSTAT & SPI_SR_TXEPT) == SPI_SR_TXEPT) {
            break;
        }
    }
    while (1) {
        if((SPI2->CSTAT & SPI_SR_RXAVL) == SPI_SR_RXAVL) {
            return SPI2->RXREG;
        }
    }
}
...
void SPIM_Init(u16 spi_baud_div)
{
    ...
    GPIOB->CRH &= ~((GPIO_CNF_MODE_MASK << GPIO_CRH_CNF_MODE_12_Pos) | \
                    (GPIO_CNF_MODE_MASK << GPIO_CRH_CNF_MODE_13_Pos) | \
                    (GPIO_CNF_MODE_MASK << GPIO_CRH_CNF_MODE_14_Pos) | \
                    (GPIO_CNF_MODE_MASK << GPIO_CRH_CNF_MODE_15_Pos));
    //set PB12,PB13,PB14,PB15 default value

    GPIOB->CRH |=  ((GPIO_CNF_MODE_AF_PP << GPIO_CRH_CNF_MODE_12_Pos) | \
                    (GPIO_CNF_MODE_AF_PP << GPIO_CRH_CNF_MODE_13_Pos) | \
                    (GPIO_CNF_MODE_AF_PP << GPIO_CRH_CNF_MODE_15_Pos));
    //PB15,PB13push-pull output
    GPIOB->CRH |= (GPIO_CNF_MODE_FLOATING << GPIO_CRH_CNF_MODE_14_Pos);
    //PB14 float input

    GPIOB->AFRH &= ~((GPIO_AF_MODEMASK << GPIO_AFRH_AFR12_Pos) | \
                     (GPIO_AF_MODEMASK << GPIO_AFRH_AFR13_Pos) | \
                     (GPIO_AF_MODEMASK << GPIO_AFRH_AFR14_Pos) | \
                     (GPIO_AF_MODEMASK << GPIO_AFRH_AFR15_Pos)) ;
    //set PB12,PB13,PB14,PB15 to default AF value

    GPIOB->AFRH |=  ((GPIO_AF_MODE0 << GPIO_AFRH_AFR12_Pos) | \
                     (GPIO_AF_MODE0 << GPIO_AFRH_AFR13_Pos) | \
                     (GPIO_AF_MODE0 << GPIO_AFRH_AFR14_Pos) | \
                     (GPIO_AF_MODE0 << GPIO_AFRH_AFR15_Pos));
    //set PB12,PB13,PB14,PB15 to AF0
    ...
    SPI2->CCTL |= SPI_CCR_RXEDGE;                                               //start high speed  mode
    SPI2->SPBRG = spi_baud_div;                                                 //2 fractional frequency,baud  should 72M/2=36M
    SPI2->CCTL &= ~SPI_CCTL_LSBFE;                                              //MSB
    SPI2->GCTL |= SPI_GCTL_SPIEN;                                               //enableSPI
    SPIM_TXEn();
    SPIM_RXEn();
    SPI2->GCTL |= SPI_GCR_SPIEN;
}
