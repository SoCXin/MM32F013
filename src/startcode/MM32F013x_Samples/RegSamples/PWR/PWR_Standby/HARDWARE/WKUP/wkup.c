...
void WKUP_Init(void)
{
    RCC->AHBENR |= RCC_AHBENR_GPIOA; //firstly enableexternal IO PORTAclock
    RCC->APB2ENR |= RCC_APB2RSTR_EXTI;
    GPIOA->CRL &= ~(GPIO_CNF_MODE_MASK << GPIO_CRL_CNF_MODE_0_Pos); //PA0set  input
    GPIOA->CRL |= (GPIO_CNF_MODE_FLOATING << GPIO_CRL_CNF_MODE_0_Pos);
